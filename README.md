...
- 从原版、档案启用的官方 DLC、Workshop Mod 和本地 `mods` 内容读取可计数物品、饰品、人物与怪癖；禁用内容不会进入目录。物品目录会先检测当前档案状态：小镇中读取 `wallet` 的金币、祖产、星晶、建筑图纸、Mod 货币，以及 `estate_items.items` 的血酿、邀请函和 Mod 庄园物品；副本中改为读取当前 `party.inventory.items`，显示火把、解毒剂、食物、宝石、祖产、任务物品和活动 Mod 的同类定义。同一定义可以合法地同时存在于两种目录中，程序会分别读取各自存档里的数量，不会因它可在小镇修改就把它从副本目录排除。饰品副本（包括当前背包中已经携带的饰品）仍由独立饰品功能处理，任意进度数值不会混入物品目录。Mod 物品会跨活动内容建立“技能、英雄、怪物、任务、城镇事件、建筑或配给 → 掉落表 → 物品”的引用链；存档不存在且从任何玩法入口不可达的旧定义默认隐藏，可用“显示未使用定义”重新显示。属于数量功能范围且在存档中实际存在的条目始终显示。物品、人物、怪癖和饰品同时解析 `schinese` 与 `english` 名称；缺少某种语言时显示 `—`，不会伪造翻译。
