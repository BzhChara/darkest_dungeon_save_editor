...
public partial class MainWindow : Window
{
    ...
    private bool _syncInvalidated;
    private int _catalogOperationGeneration;
    private bool _battleMaintenanceDeferred;
        ...
        string contentFingerprint, SaveEditorLocations? locations = null)
    {
        ...
        _catalogSnapshotReader = new(content, items, codec, gameDirectory, workshopDirectory, localModDirectory,
            workspaceRoot: locations?.WorkspaceDirectory, initialContentFingerprint: contentFingerprint);
        _battleMaintenance = new ManagedBattleEncounterBridgeService(codec, locations);
    ...
    private void StopProfileSync()
    {
        ...
        _battleMaintenanceDeferred = false;
    ...
    private void RequestProfileSync(bool invalidatePreview = true)
    {
        ...
        if (invalidatePreview)
        {
            ...
            _syncReady = false;
            // Do not clear objects used by an in-flight commit.
            if (!IsBusy)
            {
            ...
            while (_syncRequested && !IsBusy && generation == _catalogGeneration)
            {
                ...
                if (unchanged)
                {
                    if (!((_battleMaintenanceDeferred || _maintenanceRetryPending) &&
                          maintenance is { CanRetryDeferredMaintenance: true } && maintenanceGameDirectory is not null))
                    {
                        CompleteUnchangedSync(snapshot);
                        continue;
                    }
                    // Retry failed maintenance as a read-only inspection. A locked history
                    // file must not repeatedly invalidate previews or freeze the controls.
                    EditorBattleMaintenanceResult? inspection = null;
                    Exception? inspectionError = null;
                    try
                    {
                        inspection = await Task.Run(() => maintenance.ReconcileAsync(snapshot.Content,
                            maintenanceGameDirectory, maintenanceLocalModDirectory, token, inspectOnly: true), token);
                    }
                    ...
                    {
                        _maintenanceRetryPending = false;
                        _battleMaintenanceDeferred = inspection!.RetryWhenGameExits;
                    }
                    if (inspection is not { RequiresWrite: true })
                    {
                        CompleteUnchangedSync(snapshot);
                        continue;
                    }
                }
                // From this point maintenance can write, and catalog publication replaces UI objects.
                // Enter exclusively on the dispatcher, after checking for a user operation.
                _syncApplying = true;
                _syncReady = false;
                InvalidatePreparedEdit();
                BattleMapPanel.DismissProfileMenu();
                ...
                try
                {
                    if (maintenance is not null && maintenanceGameDirectory is not null)
                    {
                        ...
                        {
                            _maintenanceRetryPending = false;
                            _battleMaintenanceDeferred = result.RetryWhenGameExits;
                        }
                ...
                {
                    if (generation == _catalogGeneration) _syncApplying = false;
                    UpdateEnabledState();
                }
            }
        }
    ...
    private bool IsPublishedSnapshot(ProfileCatalogSnapshot snapshot) =>
    ...
    private void ReportMaintenanceError(Exception error, ManagedBattleEncounterBridgeService maintenance)
    {
        ...
        _battleMaintenanceDeferred = !maintenance.CanRetryDeferredMaintenance;
    }
