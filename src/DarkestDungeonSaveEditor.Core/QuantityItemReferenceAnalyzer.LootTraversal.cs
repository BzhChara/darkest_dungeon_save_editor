        ...
        Dictionary<string, List<string>> activeEvidence, Dictionary<string, List<string>> incompleteEvidence)
    {
        ...
        while (queue.TryDequeue(out var state))
        {
            ...
            foreach (var variant in variants)
            {
                ...
                var chain = EditorText.Format("QuantityItemReferenceAnalyzer_LootTraversal_001", state.Evidence, state.Code.Name, variant.Source);
                if (!variant.SelectionVerified) chain += EditorText.Get("QuantityItemReferenceAnalyzer_LootTraversal_002");
                var uncertainChain = chain + EditorText.Get("QuantityItemReferenceAnalyzer_LootTraversal_003");
                ...
                foreach (var item in variant.ConflictedItemKeys) AddEvidence(incompleteEvidence, item, chain + EditorText.Get("QuantityItemReferenceAnalyzer_LootTraversal_004"));
