...
internal static class HeroEquipmentProgression
{
        ...
        List<string>? warnings = null)
    {
        ...
        if (weapon.Rank != profile.WeaponRank || armour.Rank != profile.ArmourRank || armour.Hp != profile.ArmourHp)
            throw new InvalidOperationException(EditorText.Format("HeroEquipmentProgression_001", hero.Id));

        foreach (var (label, ranks, target, current) in new[]
        {
            (EditorText.Get("HeroClassCatalog_Progression_005"), equipment.Weapon, targets["weapon"], weapon),
            (EditorText.Get("HeroClassCatalog_Progression_006"), equipment.Armour, targets["armour"], armour)
        })
        {
            if (ranks.Skip(current.Rank + 1).Any(rank => rank.RequirementCode == 0 || purchases.Any(p =>
                ...
                warnings?.Add(EditorText.Format("HeroEquipmentProgression_002", label, current.Rank));
        }
    }
}
