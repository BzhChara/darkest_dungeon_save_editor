...
Upgrade tree selection still uses the last exact tree definition. Purchase persistence must hash the **same raw tree ID** and keep the same requirement code selected by generation, without `Trim()`. A tree `hero.skill ` is different from `hero.skill`. Stagecoach generation and JSON/DSON purchase writes now retain that identity; real hash collisions, duplicate purchases, unsupported codes and missing dependencies remain rejected. No change is made to skill/Buff/quirk merge rules or the recruitment pool policy.
