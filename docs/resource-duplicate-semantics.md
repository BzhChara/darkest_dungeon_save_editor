...
Initial research dates: 2026-09-08 and 2026-09-09. That evidence applies to Windows x64 build 27890, with `Darkest.exe` SHA-256 `35E5A653279992564809FF8406FEBD5A02A7D6961044781B1296B38A7096F59B`. Later sections explicitly identify evidence relocated in the updated executable.
...
- **Mode conditions:** runtime `in_mode` compares the actor's current mode hash with the Buff rule-string hash (`0x140473CFE`–`0x140473D0B`). Enumerate conditions by these hashes, including collisions that actually represent the same native condition. `Mode`, `mode` and ` Mode ` have different hashes and cannot cancel each other's HP changes. Rule thresholds use adjacent float values for boundary checks. Mode/light conditions remain inapplicable during stagecoach creation because the required raid context is absent, including when inverted.
- **Initial affliction condition (2026-09-15):** `afflicted` is applicable to a Hero without raid context (`0x14047380B`), tests the Hero state at `+0x11E0` (`0x14047DF5E`), then applies `is_false_rule` (`0x140473FFF`–`0x140474009`). A fresh unafflicted hero therefore activates an inverted `afflicted` HP Buff. Initial generation reuses the recognized-condition evaluator with `hasTrinkets=false`, `isAfflicted=false`, and null mode/light; missing context remains distinct from a known false condition. Known initial active effects determine full `current_hp`; every reachable recognized state still participates in non-positive-HP validation. No Buff instances are serialized for these quirks. [Audit](change-history/initial-hp-condition-audit-2026-09-15.md), [implementation and validation](change-history/initial-hp-condition-fixes-2026-09-15.md). Evidence is pinned native static control flow plus isolated editor contracts, not a new live-game capture.
...
| `upgrades/**/*.upgrades.json` | Final tree and final per-code `requirements[].currency_cost[]`; see section 25 | Town |
...
The entry category and item-reference identity path additionally follow the consumer-specific hashes described in [section 29](#29-loot-item-reference-identities-2026-09-16), verified against the updated executable. The above addresses remain evidence for the earlier pinned build.
...
The existing last-complete-tree rule and a tree's internal requirement rule are separate selection steps. The x64 build 27890 requirement lookup `0x1404703D0` retains the last full tree matching the requested native hash. `0x140470460` then loads requirements into a map keyed by the first UTF-8 byte of `code`: `0x140470CD0` returns the existing node on a repeated key, and the caller replaces its costs, prerequisite list and resolve-level field (`0x1404707AF`, `0x1404707D0`, `0x1404707D8`). A repeated code with a different resolve level is therefore a final assignment, not a conflicting definition.

Both hero generation and upgrade-cost reference analysis use `NativeUpgradeRequirements` to choose the final per-code rows before reading their fields. For example, code `1` requiring level 3 and then level 1 ends at level 1; reversing the records ends at level 3. One personal purchase remains for this code. Hero persistence still rejects a winning authored code that the DSON codec cannot represent losslessly; an overwritten nonrepresentable spelling does not invalidate a later valid winner for the same native byte. Existing tree/purchase hash collision guards and skill/equipment availability checks remain.

Quantity references first select final complete trees across all consumer-eligible files, using the existing native file slots and overlays. They then collect only the final requirements' `currency_cost` entries. A new tree with an empty requirements array or a final row with empty costs removes the old cost path. Other distinct codes, distinct trees and independent event/quest/etc. uses remain independent evidence. JSON property lookup remains first exact member; this is different from last matching array-record selection. The reference catalog is a possible-use analysis, not a simulation of every building trigger, price condition or quest lifecycle.

Missing/unreadable upgrade files or unidentifiable tree records can hide replacements, so upgrade-derived uses remain uncertain until the identities can be resolved. A known-ID tree with unsupported requirements occupies its final slot without exposing old costs; other known healthy trees still contribute. Existing saved items, official items and manual hidden-item edits keep their normal rules. No save migration, cached legacy-tree fallback or Mod-specific exception is introduced.

The original audit's 60 scenarios are formal regression inputs, now tested directly against corrected generation/reference results. Additional coverage checks first JSON members, native code bytes/hashes/NUL termination, malformed/missing files, independent references, content-only refresh and winning-code persistence guards. See [audit evidence](change-history/upgrade-requirement-consumer-audit-2026-09-15.md) and [implementation/validation](change-history/upgrade-reference-fixes-2026-09-15.md). Native evidence is static analysis of the fixed executable; this change does not claim another live-game test.

## 26. Non-actor text reference consumers (2026-09-15)

Manifest eligibility and a supported root directory are not proof that a file supplies item references. The shared reference filter now applies known consumer queries to discovery and missing-file diagnostics alike. Provision, town-event, estate, quest, district, building and upgrade references must satisfy their JSON queries; Curio references must satisfy the type-library CSV query. Arbitrary `.darkest` files in those roots are excluded, regardless of their item-looking fields, basename or nesting. The filter identifies known directory families without case folding the native query itself: a wrong-case Mod directory does not become an unverified text source after its query fails. Physical directory aliases and native unescaped filename dots retain their established rules.

For example, `campaign/provision/notes.darkest` containing `notes: .type estate .id audit_token` cannot make that item raid-reachable. Its missing manifest entry cannot make unrelated items analysis-incomplete either. `campaign/town_events/notes.darkest` behaves the same way for town references. Native evidence is the fixed build 27890 provision query at `0x140450438` / `0x140450444` and event query at `0x1403E877E` / `0x1403E878A`, as detailed in the [audit](change-history/nonactor-item-reference-audit-2026-09-15.md). No new live-game test is claimed.

Other, unmodeled non-actor text sources retain their existing conservative file overlay. Their `.type/.id`, `.item_id`, `.use_item_id` and recognized loot-code clues are now uncertain evidence, not confirmed gameplay roots. Comments and overwritten scalar fields retain native text-reader semantics. An uncertain root stays uncertain through nested Loot tables; an independent proven root can still confirm the same item. Only matching clues acquire this uncertainty, so unrelated definitions keep their own status. Unreadable or missing potentially relevant sources retain the existing context-specific incomplete-analysis handling.

Canonical hero `extra_battle_loot` / `extra_curio_loot` and monster `loot` records remain confirmed consumers; unknown actor records remain ignored. Known-but-unverified JSON structures also retain their previous uncertainty behavior. The change does not modify resource IDs, Mod priority, quantity persistence, stack limits, existing saved entries, encounter indexes or Bridge maintenance. See [implementation and validation](change-history/nonactor-item-reference-fixes-2026-09-15.md).

## 27. Missing providers and wallet definition identities (2026-09-15)

Manifest eligibility, native query matching, result-slot merging and opening the selected provider are separate steps. A matching manifest entry is retained through slot/provider resolution when its physical file is absent. An unreadable selected file produces a diagnostic or an unavailable consumer; discovery must not expose a lower Mod file that the native slot rules replaced. This applies to inventory, trinkets, shared queries, hero metadata, encounters, map resources and localization. It does not turn additive flags-1/flags-9 queries into a universal same-path winner rule. A missing lower file masked by a readable effective provider need not block that provider, and unlisted or query-ineligible files still do not participate.

Quantity catalogs record effective definition read failures. A saved entry with no resolved definition remains visible but cannot be treated as an editable save-only residue while that scan is incomplete. Saved-amount refresh carries this state to newly seen residue; service preview and commit independently recheck it. Restoring the files and completing a fresh scan restores ordinary residue behavior. Known resolved rows retain their existing validation. Missing encounter tables cannot yield a guessed append position or maintenance renumbering. Profile and battle change fingerprints represent stable missing states and detect byte restoration without a manifest edit; write guards do not infer validity from those fingerprints.

The quantity reference analyzer's separate discovery path also retains eligible missing manifest candidates through provider resolution. When a winning Loot file is missing, a lower same-path table cannot establish confirmed usage for an otherwise readable item definition. The existing incomplete-reference status applies; restoring the provider can confirm the reference again. Shared profile synchronization discards an unreadable prop catalog's stale choices and records the error while publishing other valid catalogs, matching initial load behavior.

Inventory lookup still selects the first definition for a raw `(type,id)` key. Town wallet aggregation happens afterward using the persisted key: multiple gold/shard variants, and an heirloom whose ID maps to the same wallet type, may share one balance without a provider conflict. The representative definition is retained for display, combined source provenance is retained, and genuine native-hash/provider conflicts remain guarded. Hash-collision detection compares raw definition identities before aggregation, so two different IDs with the same native hash cannot disappear into a shared wallet row. Raid stacks retain their exact `(type,id)` identity and each winner's stack limit. No save migration or legacy format compatibility is introduced.

Evidence is the [historical audit](change-history/inventory-provider-and-wallet-audit-2026-09-15.md), the previously established resolver/open rules, and isolated catalog/service/DSON regression tests in `InventoryProviderContractTests.cs`, `MissingInventoryProviderContractTests.cs` and `MissingResourceProviderContractTests.cs`. See [implementation and validation](change-history/inventory-provider-and-wallet-fix-2026-09-15.md). This corrects editor behavior against the existing contract; it does not claim a new live-game experiment covering every native I/O failure.

## 28. Reference completeness and wallet consumer identities (2026-09-16)

Missing-file diagnostics precede provider resolution and must not set global reference-completeness flags. A readable higher-priority upgrade file can replace a missing lower-priority file at the same path; its final tree costs remain usable evidence, and an unrelated unused item remains suspected-unused. If the effective winner is missing, it still blocks fallback to the replaced lower file and leaves the applicable analysis incomplete. Additive queries retain every participating input, so a missing district input can still matter. Consumer eligibility and town/raid boundaries are unchanged. This is a correction to the separate reference-analysis path, not a change to native file ordering or an instruction to suppress installation diagnostics.

Three identities remain distinct: raw inventory `(type,id)`, persisted town wallet type, and display ID. Keep all effective first-match raw definitions for reference analysis, then merge their evidence by catalog key. Typed rewards must name an actually selected raw pair. For example, a reward for `gold/variant` can establish a use for the gold wallet, but a reward for undeclared `gold/""` cannot substitute for that definition. Similarly, `heirloom/gold` and `gold/""` are distinct raw definitions even if they address the same balance.

Known currency consumers—upgrade costs, event currency fields, estate currency declarations, district currency costs and quest currency tables—use the established native hash comparison against the wallet's persisted type. An event naming `gold` therefore contributes evidence to the wallet represented by `gold/variant`; an event naming only `variant` does not. Estate currencies use their item ID. This does not change unverified free-text clues into confirmed currency consumers or change raid-stack identity.

Evidence and official provenance from all selected raw variants are combined for a shared wallet. Discarded later definitions of the same raw pair do not regain influence. Saved balances still remain visible; this change neither changes quantities nor migrates saves. Formal coverage consists of 60 provider scenarios, 76 wallet-reference scenarios and additional recovery/provenance/raid controls in `QuantityReferenceProviderContractTests.cs` and `QuantityReferenceIdentityContractTests.cs`. Four isolated commits verify the actual wallet keys after DSON readback. See [original audit](change-history/quantity-reference-identity-audit-2026-09-15.md) and [fix record](change-history/quantity-reference-identity-fix-2026-09-16.md). These validate editor behavior against established resource contracts; no new live-game experiment is claimed.

## 29. Loot item reference identities (2026-09-16)

Loot entry dispatch reads the first exact JSON `type` member, hashes its complete C string (stopping at NUL), and compares that hash to the `item`/`table` categories. This category field is not a 63-byte item buffer. For item entries, `data.type` and `data.id` each pass through a 64-byte `strncpy_s` buffer with `_TRUNCATE`, retaining at most 63 UTF-8 bytes before the native type/ID hash lookup. Hash the raw retained bytes even if the boundary splits a multibyte character; do not decode and re-encode a replacement character. The referenced definition's hash still uses its complete C string, not a truncated copy of its authored ID.

Examples: `item\u0000tail` dispatches as an item entry; an item ID `loot_a\u0000tail` resolves to the normal `loot_a` definition. With only `(estate, Az)` defined, `(estate, BE)` reaches it because both IDs hash to 3567. This does not turn case-insensitive matching back on: other casing/whitespace with a different hash remains distinct. An absent blank-ID definition cannot be substituted merely because another variant shares its town wallet.

The editor indexes already selected definitions by their native hash pairs for this Loot consumer. If matching definitions have unresolved provider or hash conflicts, the Loot path contributes uncertain evidence rather than choosing an arbitrary winner; the existing read-only guard remains. Definition conflicts and uncertain weights retain separate diagnostic reasons; both are reported when both occur. An independent confirmed reference or official provenance retains its normal classification. Same-path provider resolution, raw duplicate-definition first-match values, JSON first-member behavior, entry weights, conditional table selection, town/raid reachability, saved-item visibility, and transaction guards remain in force. Display and persistence retain the selected definition's exact identity. Other typed-reference consumers keep their existing rules pending consumer-specific evidence; there is no global string normalization or save migration.

Updated x64 executable SHA-256: `d83a6d578d059c8c9a0bd70453da65d85974715a616872c2dfeff1a5c9ab3a49`. String/RIP references and PE function ranges independently locate `LootTable` load at `0x1404449F0`, entry category hashing at `0x140445000`, `ItemEntry::LoadFromJsonValue` at `0x140445620`, and its called item reader at `0x1405D2310`; the latter copies the buffers at `0x1405D25FA–0x1405D262B` and compares definition hash pairs from `0x1405D26F3`. The old executable-address guard was not disabled. See [audit and native evidence](change-history/historical-rule-audit-34-2026-09-16.md).

`LootItemIdentityContractTests.cs`, included in `--loot-references` and the full suite, covers 26 cases for each of local, Workshop and DLC-prefixed sources, plus conflict refusal, duplicate-definition values, unchanged-manifest content refresh and three actual DSON commits using the definition ID rather than its reference alias. This is static native corroboration and isolated editor execution, not a new live-game drop experiment. See [fix and validation](change-history/loot-item-identity-fix-2026-09-16.md).

## 30. Trinket dependencies and HP Buff enum identities (2026-09-17)

The updated x64 executable identified in section 29 loads trinket entries at `0x1404F5010–0x1404F73D9`. Ordinary `buffs` references hash their entire C strings up to NUL. An absent Buff sets the entry's validity flag to false, and the entry is omitted from the loaded vector. All `hero_class_requirements` likewise compare complete C-string hashes against registered actors. These checks precede first-matching trinket-ID selection: if an earlier entry has missing dependencies and `quest_uses=2`, while a later valid entry has `quest_uses=7`, the latter supplies 7. A valid reference alias must not cause the earlier entry to be skipped: with only hero `Az` registered, `BE` and `Az\u0000tail` reach that same class hash, while `az` does not.

`TrinketBuffDependencies` reads only admitted Buff files through existing query/overlay resolution. Its hash set answers existence; it does not combine attributes or change the established last-whole-definition policy in HP analysis. An empty readable winning file hides its overridden lower Buffs. A missing/unreadable winning file cannot prove absence: the earlier dependent trinket remains read-only, preserving its position. Independent known-present Buffs remain present despite unrelated unreadable files; unresolved provider discovery/order is stronger uncertainty because it may change the effective files. A trinket without those dependencies remains usable. Ordinary Buff existence is separate from natural loot references and from stateful lists or the conditional `remove_if_not_active` check, whose complete coverage is not claimed here.

Preview and commit already reload the trinket catalog. They now use these same dependency results, so changes to Buff bytes alone can invalidate an old selection, including reappearance of an earlier valid entry. The existing post-replacement validation and displaced-file recovery handle detected changes during a write; no new saved fields or old-save migration were added.

Enabled Mod mappings omitted upstream must also remain uncertain. Resolver-backed snapshots compare configured Mod entries to resolved sources, including unmapped/ambiguous local titles and unavailable Workshop installations; commit carries the existing `Resolution` context into its catalog reload. A hypothetical explicit source list does not claim this saved-profile mapping. Formal tests use `ResolveAsync` and check both preview rejection and commit-context retention, with a successfully resolved Mod as the positive control.

The Buff JSON loader at `0x1404A51D0–0x1404A65C5` copies `stat_type` and `rule_type` into 64-byte buffers, hashes the retained C string, and looks up the corresponding enum. The editor resolves the two supported HP operations and five supported conditions to canonical internal names after its existing bounded-string read. For example, `d:mbat_stat_multiply` and `b7ways` select multiply/always, giving HP 30 from base 20 and amount 0.5. Alias `no_trinkets`, inverted `afflicted`, `in_mode` and `lightabove` must also participate in reachable-state validation; simply admitting their catalog rows while leaving unknown raw condition strings downstream would be incorrect. Unknown hashes, invalid strings/numbers and reachable non-positive HP retain their guards.

Evidence is the [versioned audit](change-history/historical-rule-audit-35-2026-09-16.md), its static native control flow and isolated editor/DSON execution. No new in-game lifecycle test is claimed. Formal regressions in `TrinketDependencyContractTests.cs` and `HeroBuffEnumContractTests.cs` run with `--trinket-dependencies` and the full suite; they cover 180 dependency selection cases, 120 HP condition cases, effective-provider/read-failure boundaries and actual save transactions.
