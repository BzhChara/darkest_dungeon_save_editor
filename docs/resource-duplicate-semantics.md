...
The editor bounds camping purchase targets to 63 UTF-8 bytes before the ordinary purchase writer computes their hash. It retains the full skill identity in the equipped-skill map and leaves generic resource hashing, equipment targets, and the save schema unchanged. Catalog availability, generation and preview share this purchase-plan validation. NUL identities, a boundary cutting through UTF-8, and distinct skills truncating to the same target are explicitly unsupported. Purchase-hash collisions are also rejected in preflight as well as by the existing writer. These guards do not assert that the native byte engine rejects every such input; ambiguous targets are not silently merged. Repeated declarations/references of the same exact skill remain one purchase.
...
The same generator drives catalog availability, preview and pre-commit purchase revalidation. Save schema, generic hashing, equipment rules, skill selection and the strict-majority policy for genuinely tree-less skills remain unchanged. See the [audit evidence](change-history/combat-upgrade-consumer-audit-2026-09-11.md) and [implementation/validation record](change-history/combat-upgrade-consumer-fixes-2026-09-11.md).
...
- `0x1404AF1EC`–`0x1404AF23C` constructs `dungeons/%s/%s.props.darkest` using the region ID twice. The editor now resolves only these canonical paths through the established manifest, Mod-priority and enabled-DLC rules. The native `arena` exclusion is retained. An arbitrary extra basename or nested backup copy is not another pool. This rule does **not** change `.mash.darkest` multi-file encounter tables.
...
This addition is static analysis of the pinned build plus isolated executable editor contracts, not a new live-game experiment. The loader `0x1404D8770` first opens the three root paths in this fixed order: `props/prop_definitions.json`, `props/obstacle_definitions.json`, `props/trap_definitions.json` (`0x1404D87A0`–`0x1404D87D4`). It then enumerates subdirectory prop JSON, subdirectory trap JSON, subdirectory obstacle JSON, curio type CSV, and curio mapping CSV. Each enumerated stage preserves the effective native file sequence. The root paths do not participate again as subdirectory entries. Same-path Mod priority is resolved before these consumers run; it is not applied a second time to duplicate resource names. Enabled-DLC aliases are classified by the mounted relative path, not by absolute filesystem depth.
...
Roster variables use a canonical open of `campaign/roster/roster.variables.json` (`0x140468711` / `0x14046877B`). Resolve same-path providers first, then compare the winning file's mounted path against that canonical path. Strip only an enabled DLC prefix; preserve the original authored path for reading and diagnostics. An effective DLC file can therefore supply `resolve_level_thresholds` without being mistaken for a missing root file. A nested `campaign/roster/notes/roster.variables.json`, an unlisted Mod file or a file inside a disabled DLC prefix does not qualify. Invalid winning thresholds retain the level-zero-only fallback and do not expose an earlier valid provider. Canonical Windows aliases and existing case-only mount diagnostics are preserved.
