...
- Current status: the user confirmed the title animation, readability, and stable wordmark as normal; the historical defects below are no longer release blockers
...
- `饰品 / TRINKETS` and `人物 / HEROES` tab headers are centered and fully visible.
...
4. trinket and hero searches producing zero, one, few, and many rows;
...
There is no known title-animation or main-window visual blocker in this record. Codex no longer substitutes its own desktop-app operation for the user's manual test, so future WPF, system-DPI, or window-management changes still require the user's final visual acceptance.
