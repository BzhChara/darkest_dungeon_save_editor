...
- Resolve a curio pool ID through effective `curios/**/*curio_props.csv` (including root files), then require its authored `Curio Type` in effective `*curio_type_library.csv`. Prop ID and interaction-type ID may differ; use the mapping's `UI String Name` for bilingual names without inventing missing text. Follow the same manifest authority, enabled-DLC fallback and file/ID priority as other resources. Missing mappings/types, empty sprite references and ambiguous equal-priority mappings are withheld. Type-library IDs follow `ID STRING` blocks; the native COM miller-wife block legitimately has no `Nothing` label on its ID row. Parse quoted commas/multiline fields, diagnose malformed CSV rows, and never interpret a failed row as the following block's ID. This proves identifier resolution, not every texture file or scripted outcome.
- Traps and obstacles are region-bound by the effective `dungeons/<region>/...` pool path. Do not collapse identical IDs across different regions. UI filtering and preparation both enforce this restriction, including against forged source metadata or a stale selection. The menu goes directly to the existing write confirmation without a search dialog, choice submenu or redundant candidate count. One eligible resource is automatic; multiple resources use positive authored `.chance` weights. A row's weight is shared equally among its distinct `.types` IDs; duplicate eligible rows in the winning source retain their contributions. Existing file/ID precedence is unchanged. Missing, repeated, negative or non-finite `.chance` values are diagnosed and do not participate; zero weight is also excluded. Missing/scripted/ambiguous resources remain excluded before selection. Weights are normalized before summation to avoid overflow. Empty regional pools remain disabled without cross-region fallback. Selection occurs once on click; confirmation shows the chosen region/resource rather than fabricated translations, and commit uses that exact selection. Resolve IDs against the current catalog after profile-sync guard rebinding. Curio/treasure search and all transactional guards are unchanged.
- Resource IDs, curio type references, prop inheritance and hash-collision checks are case-sensitive, matching the native hash bytes. Windows file paths and region comparison remain case-insensitive. Distinct valid IDs such as `foo` and `Foo` remain separate write choices. The existing shared localization reader still folds name-key case: if two names differ only by key casing, their displayed translations can merge. That pre-existing display limitation is not a reason to bind one resource ID to the other's hash; changing the shared localization contract is outside this map-writing change.
- Trap/obstacle resources come only from active `props/**/prop_definitions.json`, `trap_definitions.json`, and `obstacle_definitions.json`, including the root files. Manifest authority, file overlay and enabled-DLC fallback match the pool scan. Follow `default_data.inherits_from.prop_type_name` to the actual `trap` or `obstacle` base resource. Missing resources, equal-priority conflicts, cycles, and definitions/ancestors with active `generate_ambush`, `teleport` or `ancestor_talk` are withheld with a diagnostic, not replaced by another region's defaults. This conservative script check includes difficulty variations; it does not simulate inheritance overrides or script execution.
...
The loaded main window now owns a shared profile watcher for game, estate, roster, town, upgrades, raid and map saves. The embedded battle view disables its independent watcher and joins the same read/write coordination. Town/raid quantity caches and stale previews follow this snapshot automatically. Definition catalogs rebuild when the semantic Mod/DLC/mode configuration changes, not simply whenever the game-save file hash changes. Selected content paths still require explicit loading. A single main-window sync indicator replaces the embedded map's duplicate live status. See [automatic synchronization rules](content-save-rules.md#automatic-synchronization-of-the-selected-profile).
...
6. Current-pool fixed-boss and ordinary encounter placement. **Implemented behind exact single-contributor table guards. User-operated encounter tests and the recorded room-attachment tests provide bounded live evidence, not exhaustive coverage of every formation.**
