...
This document records the agreed product behavior for the battle-map workspace. The current implementation milestone is **real-map display, transactional delete and party movement, direct save-based force-town recovery, guarded current-table battle placement, guarded room-battle attachment editing, and a persistent profile-scoped Encounter Bridge for eligible global encounters**. `删除`, a proven battle placement, and a guarded room-battle attachment edit may replace `persist.map.json`, while `移动队伍到此` may replace `persist.raid.json`; these operations require the game to be closed, validate the displayed map/raid hashes as one guarded pair, perform a DSON encode/decode round trip, create a full-profile backup, and restore the edited file if final verification fails. `强制返回城镇` validates the displayed pair plus `persist.game.json`, creates the same full-profile backup, then replaces only `persist.game.json`. Standalone curio, standalone treasure, trap, obstacle, ambush-composite, and unresolved named/script create/replace actions remain preview-only. For a selected global encounter, the application stages and validates a managed local Mod, installs it under the configured local-Mod root, activates it at the top of the selected profile, then writes the resolved stable index to the map. It never removes or reorders earlier managed rows automatically.

The battle page now joins the save's static topology to its dynamic knowledge/content state and current raid position. Its map panel, rooms, corridors, exploration markers, event markers, and party marker are loaded at runtime from the user's selected Darkest Dungeon installation. Original game artwork is not copied into or redistributed with the editor. A visibly simplified fallback is allowed only when the selected installation does not contain the required asset. Exact single-file current-table battles use the guarded write path described below; unresolved create/replace choices remain previews and are discarded by the next save refresh.
...
The engine supports the unguarded room values 7 and 9. They occur in real/custom maps, so the console may expose them. However, all 99 currently installed Base and Color of Madness ordinary random-map records set `.room_curio`, `.room_treasure`, and `.room_battle` to `0 0`; those records generate ordinary room content through guarded curio/treasure controls instead. Therefore an `按原版随机` convenience action must reproduce the active map-generator mix and will normally choose a guarded room type. The explicit unguarded options remain intentional console overrides and must not be described as the Base game's usual random result.
...
| `陷阱` | 3 | effective `traps` pool | one resolved dynamic `trap` hash; no prop or mash |
| `障碍` | 4 | effective `obstacles` pool | one resolved static `obstacle` hash; no prop, trap, or mash |

There is no general corridor treasure category. A corridor chest, discarded pack, sack, or similar loot-bearing object is still a `奇物` selected from `hall_curios`; the fact that interacting with it can yield treasure does not turn its map content into room `宝箱`. The future context menu must therefore remove `宝箱` from corridor `新建` and `替换`.

Persisted hunger (`content=8`) and secret doors (`content=13`) are not generic editable corridor events. Hunger stays visually hidden and exposes only movement as already agreed. A secret door participates in hidden-room topology, so it also exposes movement only; it must not be created, replaced, or deleted through the ordinary corridor menu. The unverified ambush values 2, 11, and 12, scripted happenings (`content=5`), Mod-specific mash categories, and locked/quest door props remain outside the first content writer.
...
- Standard mash `.chance` and prop `.chance` remain evidence of the game's natural weighted pools; the explicit console picker does not roll those weights. Repeated equivalent rows therefore remain valid source rows but are presented as concrete compositions rather than a random action.
...
- The attachment picker reads all effective `room_curios` and `room_treasures` rows from Base, enabled DLC, and enabled Mods. A props row can contain several `.types` values, as in `darkestdungeon.props.darkest`; because this is an explicit picker rather than weighted random generation, each declared ID becomes one exact searchable choice and its `.chance` is not rolled or rewritten.
- The selected ID uses the game's UTF-8 base-53 name hash, including signed 32-bit JSON values. The catalog fingerprints the complete effective props-file inventory plus `persist.game.json`; a changed definition, active Mod configuration, hash collision, or ambiguous selected source invalidates the prepared edit before the map is replaced.
...
`新建` and `替换` still use the in-memory preview for unresolved standalone curios, standalone treasure, traps, obstacles, ambush composites, and other unsupported content. The separate `战斗附加内容` submenu is a real transactional writer for the guarded room values 6 and 10; it is not an alias for whole-cell `替换`. Content previews must preserve the cell's persisted knowledge state so an unknown location remains labelled and styled as unknown/global-vision rather than being falsely promoted to visited. A subsequent game save refresh restores the authoritative map content.
...
- creates one deterministic local-Mod identity per Steam user/profile and installs it under the configured local-Mod root (falling back to the game's `mods` root when the selected path is itself one Mod);
...
- copies each current effective standard table byte-for-byte the first time that target virtual path is needed and appends selected compositions as zero-weight rows of the same authored `hall`/`room`/`boss` kind;
...
- never removes rows and never reorders earlier rows. Adding another encounter can only append after the existing file, so every map reference already stored by the game remains stable;
- records source identity, source-relative path, full ordered composition, original region/difficulty, assigned index, hashes, and the non-Bridge active-content fingerprint in `ddse-managed-encounter-bridge.json`;
...
- rejects a running game, stale profile/catalog, externally edited carrier, duplicate manifest index, ambiguous target type, or an attempt to append a new row after the non-Bridge Mod set/order changed. An already recorded row remains reusable because its table and index are unchanged. It restores both the carrier and `persist.game.json` if post-write resolution fails;
...
- resolve curio, trap, obstacle, guarded-composite, named/scripted encounter bindings, or bilingual encounter localization;
...
- write unresolved content selected through the provisional picker;
...
3. Effective content catalog and deterministic room/corridor menu binding, including ordinary, roaming/special and fixed-boss encounters plus entrance, hidden-room, final-room, and authored-map eligibility guards. **Partial: exact current-table standard rows and global Bridge-eligible conditional/additional, explicit-roaming, and fixed-boss rows are bound; bilingual localization, boss-tag inference, named/script classification, and authored-map compatibility remain pending.**
4. Transactional ordinary content deletion and replacement. **Complete for delete, proven battle placement, and guarded ordinary-room battle attachment add/replace/remove; standalone props, traps, and obstacles remain pending.**
...
6. Current-pool fixed-boss and ordinary encounter placement. **Implemented behind exact single-contributor table guards; controlled live validation remains manual.**
...
8. Production Encounter Bridge Mod and cross-region/Mod fixed-boss or roaming/special placement. **Implemented as one persistent profile-scoped managed carrier with automatic local installation, profile activation/top ordering, append-only multi-encounter tables, exact-index re-resolution, duplicate reuse, rollback, and the existing guarded map placement transaction. The historical one-entry live probes validate the engine contract; the new automatic lifecycle still requires one end-to-end UI live test.**
9. Direct save-based force-town recovery. **Implemented from the established two-field crash-loop repair; real-game validation remains manual.**
...
- The room/corridor menu has no corridor `宝箱`; unsupported system/script cells expose movement only. Unresolved content categories remain previews until their complete binding catalogs exist.
...
- The entrance exposes movement only; visited ordinary rooms and corridor tiles remain editable; hidden and final rooms follow their dedicated rules.
- `战斗` separates ordinary encounters, roaming/special encounters, and fixed boss-room encounters. Bridge-eligible Base/DLC/enabled-Mod candidates are searchable in a dedicated table by composition, original region, difficulty, source, source kind, and roaming ID; `hall:` candidates remain corridor-only, `room:` candidates remain room-only, and fixed `boss:` candidates remain room-only.
...
- Runtime game assets remain read-only. Exact battle selections and guarded room-battle attachment edits use their guarded write paths; unresolved standalone `新建` and `替换` choices remain in-memory previews. `删除`, verified battle placement, room-battle attachment editing, and `移动队伍到此` use the battle-map save service, full-profile backup, paired stale-hash guards, round-trip validation, and rollback.
