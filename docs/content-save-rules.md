...
3. Discover manifest-eligible `upgrades/**/*.upgrades.json` files (including enabled DLC mounts), resolve same-path providers and retain the native file enumeration order. For each exact tree ID, use the last full definition in that sequence, including repeated trees within one file. Filenames do not identify the hero: one file may provide trees for several classes, and a partial file replaces only the tree IDs it declares. Do not select an entire file by hero compatibility or highest Mod priority after file resolution. Bind equipment using `<class>.weapon` / `<class>.armour` and skills using `<class>.<skill>`; preserve each winning tree's source and the raw ID/code strings, including case and surrounding whitespace. Do not trim them into different identities or valid single-character codes. Tags do not replace these ID lookups. A parsed winning tree with unsupported requirements or a native-hash collision remains explicitly unavailable rather than exposing an earlier tree or entering the missing-tree compatibility path. Unreferenced trees do not become personal purchases. See [upgrade-tree implementation](change-history/hero-upgrade-trees-2026-09-08.md).
   Combat-skill binding uses the first 63 UTF-8 bytes of the constructed `<class>.<skill>` query target, while registered tree IDs and the skill's own ID remain unchanged. Keep the original skill ID separately for variant lookup and implicit-schedule voting. A full-name tree beyond that boundary is not substituted for the actual bounded target. Ambiguous targets, hash aliases, NUL, split UTF-8, and combat class/skill IDs individually beyond the verified 63-byte input boundary are explicitly unavailable. See [combat purchase consumption](resource-duplicate-semantics.md#842-combat-purchase-targets-and-reachable-tiers).
4. Write every weapon, armor, and combat-skill personal purchase whose prerequisite is satisfied at the selected level. Each authored combat tree must supply a purchasable base code `0`; other eligible codes alone cannot establish unlock. A gap before a later in-range purchased tier retains the authored purchases but warns which tier the native consecutive-code lookup can actually read. Do not invent the missing code or rename letters into digits. This validation does not prohibit letter codes in generic tree definitions or equipment purchases.
