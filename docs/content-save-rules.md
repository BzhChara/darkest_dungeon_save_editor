...
Historical records cited by filename are kept locally under `docs/change-history/`; they are not versioned or included in a new clone.
...
- Timer and activation probes are read-only background checks. With unchanged content and saves they preserve enabled controls, prepared previews, inputs, status text and logs; they do not repeatedly decode unchanged saves or run maintenance. A confirmed change enters an exclusive publication/maintenance phase. A real save notification or incomplete read still pauses stale writes. Soft requests queued during a check do not themselves disable controls. An operation-generation counter discards a background result if a user operation started and finished while it was reading. Before an inspection requests foreground replacement, opening existing target metadata with Windows `DELETE` access checks sharing permission without changing bytes; a readable target may still prohibit replacement. This is a preflight check, not a guarantee against a later external lock; actual replacements retain their transaction guards.
...
- Game-running deferral carries an explicit retry-on-exit flag. Quiet polls preserve interaction while the game runs and retry maintenance after it exits even without new save hashes. Deferral because another persistent raid still owns a Bridge position waits for a relevant snapshot change instead of repeatedly interrupting the current screen. Failed maintenance retains a separate retry flag: unchanged polls first perform read-only inspection, preserving controls and previews while an input remains locked. Inspection may use disposable staging but cannot change installed packages, saves or recovery receipts. Only a successful inspection reporting pending writes enters exclusive maintenance. Interrupted recovery explicitly reports a change so readers also refresh a restored map.
- The decoded game cache has its own active-file identity. A non-game refresh may rotate the general scratch slot while retaining that cached file; a later speculative game decode must use the other location. A partial decode cannot delete the last published game cache, so restoring the prior live bytes recovers automatically.

### Resolved-source binding guards (2026-09-15)

`ResolveAsync` and `ResolveDecoded` retain the configured game, Workshop and optional local-Mod roots together with the mode/DLC/applied-Mod configuration. This context is independent of the reader's two rotating decoded files. Before preparing and committing catalog-based edits, resolve those inputs again and compare source identity, kind, load order, physical directory and DLC prefix. Renamed, duplicated or remapped enabled local titles, and newly installed enabled mounts, invalidate the earlier selection even if the old directory and definition bytes still exist. A description-only `project.xml` edit does not invalidate the binding guard. Definition, manifest, save-hash and native-index guards remain separate and still apply.

The binding context accompanies quantity/trinket/hero previews, direct and Bridge encounter guards, and map attachment guards. Source lists explicitly constructed or replaced for standalone catalog analysis and Bridge staging do not claim a saved-profile mapping; replacing `Sources` clears that context. Installed Bridge results are resolved afresh before they can be returned as writable encounters. This does not change `modfiles.txt` admission, native path queries, same-path priority or duplicate-ID semantics.
...
- A manifest-listed path that satisfies the consumer query remains a candidate even when its physical file is missing. Resolve native slots/providers before reporting effective read failures; never revive a replaced lower Mod file by discarding the missing winner during discovery. A missing lower provider masked by a readable winner does not itself block that winner. Additive queries retain their separate inputs. Refresh fingerprints record missing files as a stable state, while write validation still rejects unavailable required resources. See [missing providers and wallet identities](resource-duplicate-semantics.md#27-missing-providers-and-wallet-definition-identities-2026-09-15).
- Shared profile synchronization isolates an unreadable map-prop catalog just as initial loading does: discard its stale choices and log the error, publish other valid catalogs, and reload props after the next relevant change. This does not relax save-coherence checks or make missing props writable.
- Replay verified native file slots in `base → DLC → Mod` application order. Base initializes the list; mode-1, flags-0 alternate enumeration replaces the first existing path containing the incoming mount-relative path, using case-sensitive substring matching, or appends if no match exists. Process each mount in descending path-depth order, then ordinal filename order. Surviving unprefixed Base paths resolve their opened provider afterward; repeated reads retain repeated slots. Effect uses flags 1: erase an offset-zero match before appending, but retain a match inside a prefixed provider. Curio, nested props and District use flags 9, retaining concrete Base and alternate inputs. Keep independent query lists, canonical opens and the three Base-only `>props/...` defaults separate. File order and semantic-ID lookup are separate rules; see [the bounded merge contract](resource-duplicate-semantics.md#21-alternate-mount-result-slot-merging-2026-09-14) and [canonical opens and query flags](resource-duplicate-semantics.md#22-canonical-requests-effect-flags-1-and-district-flags-9-2026-09-14).
...
- Effective definition-file failures retain their place in that sequence. For inventory/trinket first-match lookup, an already verified first definition stays usable, but a new ID after an unreadable slot cannot be confirmed. For quirk/hero-upgrade last-match lookup, a failure invalidates earlier values until a later complete definition re-establishes that ID. Unknown upgrade-tree presence must not become a tree-less default. Uncertain definitions remain unavailable to generation, preflight and commit; saved-amount refresh preserves their status. Full reads after recovery restore availability. This extends file-read completeness, not the duplicate-ID or manifest policies; see [ordered definition failures](resource-duplicate-semantics.md#32-ordered-definition-file-failures-2026-09-23).
...
- Canonical actor lookups test the original request against case-sensitive Mod manifest keys, without removing a DLC prefix. Only the subsequent physical open uses Windows filename aliases. Base/mode/official-DLC directory devices can answer root requests from their own roots. Native hash collisions between different inventory keys, trinket IDs, hero IDs or quirk IDs remain unavailable; evolution targets cannot resolve through such a collision. Monster collisions defer affected index calculations instead of guessing a string-specific size.
...
5. **Separate file and ID rules.** Shared file resolution follows each consumer's query and open mode; a single same-path replacement rule does not describe every input list. Inventory/trinket first-match and quirk last-match semantics remain. Upgrade trees and Buffs take the last complete definition in the resolved sequence. Event results use the first matching event, while the editor's `.disease` tracking applies field assignments rather than choosing a whole Effect by Mod priority. See section 2.2 for the bounded integration; unverified cases remain guarded.
...
This is the user's adopted experiment, not proof that an originally manifest-free Mod is equivalent to the native uploader output, or that every resource's duplicate-ID rule is known. See implementation and validation (local record `manifest-only-loading-2026-09-08.md`).
...
- Hero colour variations count distinct mounted skin directories from all active sources, including independent skin Mods. Use each source's native directory entry point: manifest trees for Mods, physical directories for base/mode/official DLC. Letters need not start at A or be consecutive. PNG presence is not the directory-list criterion; unlisted Mod folders remain excluded. The same directory discovery feeds catalog refresh. This establishes the saved index range, not rendering completeness; see [hero selection rules](hero-generation-selection.md).
...
- The discovery fixes themselves did not remove localization compatibility. The later approved removal of XML supplementation/recovery and addition of legacy LOC support are recorded below and in the compatibility inventory (local record `content-compatibility-audit-2026-09-06.md`). Ordinary nested-directory discovery is not a deliberate fallback to old files and was not changed by those removals.
...
6. A missing language displays as `—`. Do not substitute the other language, the internal ID, a `[Simplified Chinese]` suffix, or invented translation.
...
Known item-reference consumers apply their native filename query before reading or diagnosing a missing file. Provision, town-event, estate, quest, district, building and upgrade roots require an eligible JSON resource; Curio roots require a type-library CSV. A listed `campaign/provision/notes.darkest` or `campaign/town_events/notes.darkest` neither proves item use nor makes analysis incomplete when absent. Directory-device case rules and native wildcard dots still apply to valid queries. Other unverified non-actor text sources can supply only uncertain item/loot clues, never confirmed references merely from `.type/.id`, `.item_id`, `.use_item_id` or `.code`. Uncertainty follows nested Loot tables and does not override an independent confirmed use or contaminate unrelated definitions. See [non-actor reference consumers](resource-duplicate-semantics.md#26-non-actor-text-reference-consumers-2026-09-15).

Loot entries require a present, positive native float32 `chances` value before they can establish item or subtable references. Missing, zero, negative (including negative overflow) and positive-underflow-to-zero weights are skipped; wrong types or positive nonfinite weights retain only uncertain evidence along a reachable path. Same-ID loot tables are ordered variants, not a union of entries: the first table matching difficulty, dungeon, infestation phase and inclusive week bounds wins, even when empty. Nested tables inherit the selected context, so incompatible parent/child conditions cannot form an active path. This is a possible-use catalog across contexts, not a filter for only the current raid's difficulty. See [loot selection rules](resource-duplicate-semantics.md#24-loot-entry-weights-and-ordered-table-variants-2026-09-15).
...
An absent Mod-only definition that is unreachable in the selected context is hidden by default as suspected unused. “Absent” means that no physical matching entry exists in the current target save; it does not mean that the summed amount happens to be zero. `Show hidden items in current context` switches the table to an exclusive view of those hidden definitions; clearing it restores the normal context-reachable and persisted rows. Any in-scope quantity entry physically present in the selected target save remains visible even when unreachable or amount zero; carried `trinket` entries are the explicit exception and never enter this catalog. A missing provider is shown as save-only. Failed relevant reference reads and malformed or missing relevant active files fail open as analysis-incomplete rather than hiding uncertain definitions. Binary starting-save templates remain excluded from reachability roots.

Event and Loot reference analysis preserves failed effective-file positions. A later event or conditional Loot variant cannot claim confirmed use if an earlier unreadable or unparseable file may supersede it. Earlier confirmed results and independent provision/use evidence remain valid; uncertainty propagates through nested Loot edges. Amount-only refresh retains the classification, while a successful content reread can restore it. These are reference metadata rules, not new quantity-write prohibitions or changes to visibility for analysis-incomplete items. See [ordered reference read failures](resource-duplicate-semantics.md#34-item-reference-certainty-after-event-and-loot-read-failures-2026-09-23).
...
Select raw inventory definitions by exact `(type,id)` and the established first-match rule before aggregating town wallet entries by their persisted identity. For example, `gold/""`, `gold/variant` and `heirloom/gold` may all address the one `wallet.type=gold` balance; distinct authored IDs alone do not make that balance conflicted. Preserve combined provenance and genuine provider/hash uncertainty. In a raid, these definitions remain separate stacks with their own limits; estate-item identities also remain distinct.

Reference analysis retains every selected raw definition until its evidence is mapped to the catalog row. The direct typed-reference paths keep their exact `(type,id)` matching; they do not invent a blank-ID definition or a wallet-type alias. Loot item references instead follow the verified consumer: C-string entry-category hashes and 63-byte UTF-8 type/ID buffers, followed by matching native definition hashes. These reference conversions never rename or truncate the selected definition or saved identity; ambiguous providers/hashes supply only uncertain loot evidence and retain their write guards. This rule is specific to Loot, not a claim that all JSON consumers share the same buffers. Known currency consumers match the native hash of the persisted wallet type (or the item ID for estate currencies), never a variant's display ID. Any effective raw variant can contribute evidence to the shared wallet, and official provenance is combined across those variants. A missing file listed in the manifest can remain an installation diagnostic, but only missing effective inputs affect reference completeness after provider resolution. Additive inputs still participate independently. See [reference completeness and wallet identities](resource-duplicate-semantics.md#28-reference-completeness-and-wallet-consumer-identities-2026-09-16) and [Loot reference identities](resource-duplicate-semantics.md#29-loot-item-reference-identities-2026-09-16).

If an effective inventory definition file cannot be read, an otherwise undefined saved entry cannot be proven to be a genuine save-only residue. Keep its amount visible but read-only, including newly observed residue during saved-amount refresh. Both preview and commit rescan definitions before allowing save-only edits, even for cached selections or prepared edits. A complete scan after the file is restored removes this uncertainty; existing residue rules then apply. Successfully resolved independent definitions keep their normal rules.
...
- An absent class is confirmed missing only after complete actor discovery. Unmapped/ambiguous enabled Mods, missing sources or failed enumeration leave it unknown: retain the earlier dependent trinket as read-only instead of selecting a later duplicate's values. Already discovered actors remain present even if another source is unavailable or their canonical info cannot be read; registration and a usable generation template are different checks. Preview and commit preserve source-resolution context. See [dependency completeness](resource-duplicate-semantics.md#31-hero-buff-read-order-quirk-references-and-class-dependency-completeness-2026-09-17).
...
| UI text | Meaning |
| --- | --- |
| `Ordinary recruitment enabled / Manual editor creation` | The ordinary generation flag is enabled and the editor can create the class; other recruitment conditions still apply. |
| `Ordinary recruitment disabled / Manual editor creation` | `is_generation_enabled=false`; ordinary random selection excludes it, but explicit-class events such as `bonus_recruit` can still create candidates. Its complete template also supports editor creation. |
| `Natural recruitment unknown / Manual editor creation` | The template does not declare the flag, so the editor does not invent a natural-generation answer. |

The displayed `Natural quirk range` is only the positive/negative quirk range used by natural game generation. It does not constrain explicit editor selections.

### 6.3 Resolve level, equipment, and skills

1. Read `resolve_level_thresholds` from the selected profile's effective `campaign/roster/roster.variables.json` and construct level 0 through that mode's maximum. Match that exact root request against Mod manifest keys; Base/mode/official DLC directory devices retain Windows physical lookup. A prefixed Mod entry does not answer a root request. Preserve the selected source path; nested namesakes do not qualify. See [canonical requests](resource-duplicate-semantics.md#22-canonical-requests-effect-flags-1-and-district-flags-9-2026-09-14).
2. The selected level determines `resolveXp` and the eligible purchase records. Walk equipment slots in insertion order against those actual purchases, stopping at the first unmet nonzero requirement. The resulting `weapon_rank` / `armour_rank` determine armor HP; taking the highest independently level-eligible rank can skip a required earlier purchase and is incorrect.
   Equipment slots follow first insertion of each native `.name` (the first 63 UTF-8 bytes, case-sensitive); numeric name suffixes do not define rank. Repeated names update that slot and retain omitted fields. `upgradeRequirementCode` reads one raw UTF-8 byte after the last exact field and whitespace; it does not unquote strings. An omitted new code defaults to NUL (free), an omitted update inherits, and an explicitly empty field clears to NUL. NUL is not ASCII `0`. Valid unused tree requirements remain in the purchase plan. Level-zero fallback also resolves real purchases and free slots, and must not fabricate rank 0 or invalid HP. Preview and save preflight recheck the raw equipment metadata, which is included in the prepared-save catalog fingerprint. Nonfinite raw HP is serialized as a named value in this metadata only, so an invalid upper slot cannot break another class's fingerprint; generation still rejects invalid selected HP. Hero info/art/override fields otherwise retain their native record, boolean, integer-prefix and float/percentage semantics. See [equipment/text rules](resource-duplicate-semantics.md#83-equipment-rank-is-a-vector-position-not-a-name-suffix).
3. Discover manifest-eligible `upgrades/**/*.upgrades.json` files (including enabled DLC mounts), resolve same-path providers and retain the native file enumeration order. For each exact tree ID, use the last full definition in that sequence, including repeated trees within one file. Filenames do not identify the hero: one file may provide trees for several classes, and a partial file replaces only the tree IDs it declares. Do not select an entire file by hero compatibility or highest Mod priority after file resolution. Bind equipment using `<class>.weapon` / `<class>.armour` and skills using `<class>.<skill>`; preserve each winning tree's source and the raw ID/code strings, including case and surrounding whitespace. Do not trim them into different identities or valid single-character codes. Tags do not replace these ID lookups. A parsed winning tree with unsupported requirements or a native-hash collision remains explicitly unavailable rather than exposing an earlier tree or entering the missing-tree compatibility path. Unreferenced trees do not become personal purchases. See upgrade-tree implementation (local record `hero-upgrade-trees-2026-09-08.md`).
   Equipment and combat-skill binding use the first 63 UTF-8 bytes of their constructed query targets, while registered tree IDs and the hero/skill IDs remain unchanged. Keep the original skill ID separately for variant lookup and implicit-schedule voting. A full-name tree beyond that boundary is not substituted for the actual bounded target. Ambiguous targets, hash aliases, NUL, split UTF-8, and combat/equipment class or skill IDs individually beyond the verified 63-byte input boundary are explicitly unavailable. Equipment sharing a target or hash with a skill is also restricted instead of merging their purchases. This equipment rule describes normal native rank refresh; other game upgrade callers use a different buffer and are not globally changed. See [equipment consumption](resource-duplicate-semantics.md#831-equipment-purchase-codes-targets-and-reachable-ranks) and [combat consumption](resource-duplicate-semantics.md#842-combat-purchase-targets-and-reachable-tiers).
4. Write every weapon, armor, and combat-skill personal purchase whose prerequisite is satisfied at the selected level. Each authored combat tree must supply a purchasable base code `0`; other eligible codes alone cannot establish unlock. A gap before a later in-range purchased tier retains the authored purchases but warns which tier the native consecutive-code lookup can actually read. Do not invent the missing code or rename letters into digits. This validation does not prohibit letter codes in generic tree definitions or equipment purchases. The bundled DSON codec can round-trip only a single printable ASCII purchase character excluding double quote and backslash (blank codes remain invalid). Other bytes may decode as booleans; quotes/backslashes are not JSON-escaped. Catalog, generation and direct purchase writes reject these codec-incompatible codes before saving; this is an editor support limit, not a native game rule.
...
6. Camping skills use the native `<class ID>.<camping skill ID>` purchase target and requirement code `0`. The target is limited to the first 63 UTF-8 bytes; retain the full skill ID in the candidate's equipped-skill map. Unlock all recognized shared and class skills only when their targets are representable and unambiguous. A split UTF-8 sequence, NUL-containing identity, two distinct skills truncating to one target, or a purchase-hash collision blocks generation during catalog preflight and preview. Repeated references to the same exact skill still produce one purchase. This does not truncate other resource IDs or change the generic save hash. See [camping purchase identities](resource-duplicate-semantics.md#841-camping-purchase-targets-use-a-63-byte-buffer).
...
- For initial combat selection, `generation_guaranteed` requires at least one marked skill when any are available, not every marked skill. Skill pools are sampled without replacement and shortages yield the available count with a warning. Combat selection retains its configured cap; class/shared camping pools never fill each other's shortages. These selection changes do not alter purchase/unlock policy; see [hero selection rules](hero-generation-selection.md).
...
- A failed effective camping-file read or unresolved camping-file order marks the pool incomplete. Earlier first classifications can remain known, but the complete class-access and unlock set cannot be proved. Hero generation, preflight and commit reject that incomplete plan even when zero camping skills are to be equipped; the UI reports `Camping skill data incomplete`. Complete, genuinely short pools still follow bounded selection above. Refreshing after read recovery restores normal generation, and detected mid-write failures use the existing save rollback. See [supporting-definition read rules](resource-duplicate-semantics.md#33-camping-pools-effect-disease-and-recruitment-clues-after-read-failure-2026-09-23).
...
- The dialog evaluates each proposed addition together with the currently selected quirks at the selected hero level. A standalone HP failure is not a permanent prohibition: for base HP 20, selecting a constant +40 first makes a constant -30 selectable, with combined HP 30. Counts, exclusions, unresolved definitions and all reachable HP states use the same core validation as generation; currently invalid additions are disabled with their current reason. Selected rows remain removable even if removing another quirk or changing level has made the combination invalid. Confirmation still validates the complete selection. Singleton context warnings remain advisory.
- Preserve the selected definition's exact quirk ID through selection, refresh, limit counting and JSON/DSON writes. Buff, exclusion and evolution references compare full C-string native hashes; a valid alias resolves to its actual definition, while genuinely different case/space hashes remain distinct. Exact repeated definitions still use the last complete object; multiple different definitions sharing a hash remain guarded. Search formatting must not alter saved identity. Only exact `shard_hungry` selects the shard recruit pool. See [reference identity rules](resource-duplicate-semantics.md#31-hero-buff-read-order-quirk-references-and-class-dependency-completeness-2026-09-17).
...
- The categories overlap: positive counts every `is_positive=true` record, negative counts only `is_positive=false` and `is_disease=false`, and disease counts every `is_disease=true` record. A positive disease therefore occupies both positive and disease quotas but is saved once. The dialog, main-window summary and classified preview lists follow the same counting rule; the catalog may still display it in the disease category. The combined preview/confirmation list displays each exact ID once, preserving distinct case-sensitive IDs.
...
- The selection summary, generation validator and save preflight use the same catalog limits. Effective limit changes invalidate prepared previews even without a manifest change; pairing an old candidate with a refreshed catalog also reruns selection validation before saving. See implementation and evidence (local record `hero-quirk-rule-fixes-2026-09-14.md`).
...
- `random_chance` follows the native JSON number-to-float32 conversion before natural/special classification and runtime skill-quirk signal filtering. For example, `1e-50` becomes zero, matching an explicit `0`; both can expose a skill's effective `.disease` assignment as a special-quirk clue. First exact JSON member lookup and last matching quirk definitions are unchanged. A missing, mistyped or non-finite probability is unresolved (`null`), never silently changed to zero or used to claim a special-quirk skill signal. Unknown probability alone does not forbid explicit manual selection because the weight is not written into the hero. See classification fixes (local record `hero-quirk-classification-fixes-2026-09-14.md`).
...
- use the compact table reason `singleton definition limit 1`.
...
6. Resolve every target along the active evolution chain uniquely by its full C-string native hash, retaining the actual target definition's ID for display. A missing/conflicting target or an invalid downstream evolution makes the source unavailable for explicit and random generation. Valid authored cycles and death-only outcomes remain supported; this checks definitions without simulating future gameplay.
...
- A: a tested candidate with base 26 and `fragile -10%` saved `current_hp=23.4`. The game displayed maximum HP 24 and `Base 26 / Quirk -10%`, demonstrating no duplicate application.
...
The inverse-affliction rule was added on 2026-09-15 using the pinned game's static applicability/inversion branches and isolated editor preview/DSON tests. These values express the editor's full-health policy, not a new live-game candidate capture. See the audit (local record `initial-hp-condition-audit-2026-09-15.md`) and fix record (local record `initial-hp-condition-fixes-2026-09-15.md`).
...
An unreadable effective Buff file invalidates earlier resolved Buff attributes because its unseen contents may replace them. Later readable whole definitions re-establish their own IDs in native file order. Unknown provider/order discovery cannot establish these slots. Only dependent quirks become unavailable; unrelated quirks without Buff references remain usable. A missing winning manifest file never revives an overridden file, and a failed shadowed provider does not poison its readable winner. Hero preview and commit recheck the resulting catalog, including unresolved Mod context; a dependency failure during replacement uses the existing transaction recovery. Releasing a lock and refreshing restores verified values. See [ordered reads and references](resource-duplicate-semantics.md#31-hero-buff-read-order-quirk-references-and-class-dependency-completeness-2026-09-17).
...
| Quirk ID | English display name | Flat HP | Note |
| --- | --- | ---: | --- |
| `hearty_ship` | Hearty | +4 | Incompatible with `insipid_ship` |
| `insipid_ship` | Insipid | -4 | Negative flat formula has a clean ordinary-stagecoach sample |
| `koshou` | Koshou (Disease) | -3 | Disease |
| `orca_Quirk6` | — | +15 | English is genuinely absent |
| `Gabriel_Quirk2` | Agrim's Blessing | +6 | Constant |
| `Floss_Quirk3` | Endless Desire I | +4 | Evolves to II after 150 |
| `Floss_QuirkA2` | Endless Desire II | +8 | Evolves to III after 150 |
| `Floss_QuirkA3` | Endless Desire III | +12 | Evolves to IV after 150 |
| `Floss_QuirkA4` | Endless Desire IV | +16 | Evolves to V after 150 |
| `Floss_QuirkA5` | Endless Desire V | +20 | Evolves to VI after 150 |
| `Floss_QuirkA6` | Endless Desire VI | +24 | Evolves to VII after 150 |
| `Floss_QuirkA7` | Endless Desire VII | +28 | Evolves to VIII after 150 |
| `Floss_QuirkA8` | Endless Desire VIII | +32 | Terminal |
...
| --- | --- | --- |
| `Octopus_Quirk5` (Believer) | `afflicted`: `+5%` | Inactive before affliction; use `B` plus other constant modifiers |
| `Kaltsit_Quirk` (—) | a mode exists and is not `KaltsitA`: flat `+20` | No active stagecoach mode; do not pre-add 20 |
| `Ailuoli_Quirk2` (Knight of the Night) | `lightabove 1`: `-50%` | No raid-light condition in the stagecoach; do not pre-subtract 50% |
...
6. Event triggering, including return-from-the-grave events, remains a separate future feature and does not belong in the hero-generation chain.
...
## Resource file eligibility (2026-09-10)

Enabled local/Workshop Mod resources must be listed in `modfiles.txt` and match the relevant game consumer query or canonical direct-open path after enabled-DLC mounting. Being listed alone does not turn README files or notes into definitions. Base, mode and official DLC use their game roots. Consumer-specific filename patterns include native wildcard separators; a universal literal suffix filter is not equivalent. Discovery, missing-file diagnostics, dispatch, pre-write revalidation and content refresh must agree on the accepted set. See [current file-query rules](resource-duplicate-semantics.md#17-inventory-effect-curio-and-encounter-file-queries-2026-09-10) for inventory, capacity, Effect, Curio and battle queries.

Reference exclusions apply to mounted resource roots, not arbitrary directory names: `loot/inventory/a.loot.json` is still a loot resource. Unused-item analysis, same-path Mod priority and resource-specific duplicate-ID semantics remain in force. Encounter file eligibility feeds all three runtime index tables, direct placement, Bridge append and maintenance; it must not silently omit a contributing file and then authorize a shifted index.

Since 2026-09-11, item reference context also uses the mounted root after removing the exact enabled-DLC mount prefix: `curios/upgrades/...` remains a raid Curio consumer. Missing/unreadable-file diagnostics use the same context. Canonical actor files only establish references through their native loot record kinds: heroes use `extra_battle_loot` / `extra_curio_loot`, monsters use `loot`. Unknown actor records or item-looking fields in unrelated actor records are not references; starting provisions and quest items retain their JSON consumers. These actor checks do not claim full validation of every non-actor Darkest record. See [reference and region rules](resource-duplicate-semantics.md#18-reference-context-actor-records-and-directory-devices-2026-09-11).

Encounter directory casing follows the source device: physical Base/mode/DLC paths accept Windows directory aliases, while Mod manifest directory-tree queries preserve case. All sources retain the exact requested filename table ID and native wildcard separators. This does not restore manifest-free Mod discovery or change monster ID casing, file-overlay priority, or duplicate-resource lookup rules.

This distinction also applies to inventory/capacity, trinket, Buff/quirk, Effect/camping, Curio, nested prop, Loot and existing reference JSON discovery. Compare raw Mod directory names and enabled-DLC prefixes before opening physical files. Actor registration is separately filtered by the native case-sensitive hero/monster filename expressions; canonical definitions and actor item references require a registered ID. Preserve direct-open Windows aliases and broad content-change detection. See [manifest-directory and actor-registration rules](resource-duplicate-semantics.md#19-manifest-directory-queries-and-actor-registration-2026-09-11).
