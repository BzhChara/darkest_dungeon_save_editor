...
1. Read `resolve_level_thresholds` from the selected profile's effective `campaign/roster/roster.variables.json` and construct level 0 through that mode's maximum. Resolve Base/mode, enabled DLC and Mod providers first; identify the canonical file by its mounted path while preserving its authored source path. Nested namesakes do not qualify. See [progression resource queries](resource-duplicate-semantics.md#20-hero-upgrade-filenames-and-mounted-roster-variables-2026-09-11).
