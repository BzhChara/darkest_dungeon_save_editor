...
Within quest JSON, `completion_reward` and `system_config_type=quest_rewards` establish town reachability; `additional_provisions` and `system_config_type=quest_provision` establish raid reachability. These contexts propagate through nested `items_definition/items` and do not affect sibling branches. The file's `campaign/quest` location alone is not evidence that a completion reward enters the raid bag. Independent references to the same typed item in both branches keep it available in both scenes. Hero-starting fixed amounts remain independent of estate quantities.
...
7. Standalone map curios/treasures and ordinary traps/obstacles edit only the guarded map file. Room and hall curios may be selected across active regions using exact resource hashes; trap/obstacle choices are current-region only. Regional traps/obstacles go straight from the menu to write confirmation; no search dialog or choice submenu is shown. One eligible resource is selected directly; multiple eligible resources use their positive authored `.chance` weights. Invalid/zero weights and rejected resources do not participate; an empty regional pool stays disabled without cross-region fallback. Curio pool IDs must resolve through active `curio_props.csv` mappings to a type in active `curio_type_library.csv`; aliases are valid, names use the mapping's UI key, and both CSV families participate in the stale-content guard. Whole-cell replacement clears conflicting props and battle indexes, unlike the battle-attachment action, which retains the encounter. No additional Bridge or Mod activation is created for hash-addressable props. Missing/ambiguous/scripted trap and obstacle resources are diagnosed and withheld. Detailed bindings and evidence are in the [battle-map rules](battle-map-editor-design.md#standalone-map-content-2026-09-06).
...
- Logs distinguish informational messages, warnings, errors and technical trace entries. Normal no-manifest standard/enabled-DLC scanning is informational, not a load failure. Unknown catalog issues and preview risk advisories remain warnings; presentation grouping does not alter the original catalog issues or any safety guard. Status helpers preserve explicit levels. Handled discovery, preview, quirk-dialog, load and commit failures record error summaries and their exceptions; missing-file/incomplete inventory diagnostics are warnings, not ordinary scan progress.
...
- Current standard mash indexes follow first-provider file slots, native per-source order, and independent hall/room/boss counters. Bridge appends to the last contributing file of its type and validates all existing bindings before installation. Multi-file DLC mount order and unknown native row counts stay guarded. Five-or-more authored actor IDs are reported and read as the native first four; source files are preserved. See [native evidence and compatibility boundaries](encounter-runtime-order.md).
